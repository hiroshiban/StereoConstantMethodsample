...
for ii=1:1:imgNum
  imgL{ii}=double(colors(2)*ones(size(heightfield)));
  imgR{ii}=double(colors(2)*ones(size(heightfield)));
end
...
for n=1:1:imgNum
  ...
  if oversampling_ratio~=1
    imgL{n}=imresize(uint8(tmpL),1/oversampling_ratio,'bilinear');
    imgR{n}=imresize(uint8(tmpR),1/oversampling_ratio,'bilinear');
  else
    imgL{n}=uint8(tmpL);
    imgR{n}=uint8(tmpR);
  end
